...
#Activation function
activation = tf.nn.sigmoid(tf.add(tf.matmul(X, weights), base))

#loss function
loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=activation, labels=y))

#Gradient descent optimizer
optmizer = tf.train.RMSPropOptimizer(learning_rate=0.001).minimize(loss)
...
with tf.Session() as session:
    session.run(init)
    writer = tf.summary.FileWriter("./graphs2/lab3_1", session.graph)
